...
export async function listSponsorAds(db) {
  const rows = await db.prepare("SELECT * FROM sponsor_ads ORDER BY start_date DESC").all();
  return rows.results || [];
}

export async function getSponsorAdById(db, id) {
  return await db.prepare("SELECT * FROM sponsor_ads WHERE id = ?").bind(id).first();
}

export async function getActiveSponsorAd(db) {
  const rows = await db
    .prepare(
      "SELECT id, title, image_url, link_url FROM sponsor_ads WHERE enabled = 1 AND date(start_date) <= date('now') AND date(end_date) >= date('now') ORDER BY RANDOM() LIMIT 1"
    )
    .all();
...
export async function createSponsorAd(db, data) {
  const result = await db
    .prepare("INSERT INTO sponsor_ads (name, title, image_url, link_url, start_date, end_date, enabled) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .bind(data.name, data.title, data.imageUrl, data.linkUrl, data.startDate, data.endDate, data.enabled ? 1 : 0)
    .run();
  return result.meta.last_row_id;
}

export async function updateSponsorAd(db, id, data) {
  await db
    .prepare("UPDATE sponsor_ads SET name = ?, title = ?, image_url = ?, link_url = ?, start_date = ?, end_date = ?, enabled = ? WHERE id = ?")
    .bind(data.name, data.title, data.imageUrl, data.linkUrl, data.startDate, data.endDate, data.enabled ? 1 : 0, id)
    .run();
}

export async function deleteSponsorAd(db, id) {
  await db.prepare("DELETE FROM sponsor_ads WHERE id = ?").bind(id).run();
}

export async function listPrerollAds(db) {
  const rows = await db.prepare("SELECT * FROM preroll_ads ORDER BY start_date DESC").all();
  return rows.results || [];
}

export async function getPrerollAdById(db, id) {
  return await db.prepare("SELECT * FROM preroll_ads WHERE id = ?").bind(id).first();
}

export async function getActivePrerollAd(db) {
  const rows = await db
    .prepare(
      "SELECT id, name, ad_type, media_url, link_url, skip_after_seconds FROM preroll_ads WHERE enabled = 1 AND date(start_date) <= date('now') AND date(end_date) >= date('now') ORDER BY RANDOM() LIMIT 1"
    )
    .all();
  const results = rows.results || [];
  return results[0] || null;
...
export async function createPrerollAd(db, data) {
  const result = await db
    .prepare(
      "INSERT INTO preroll_ads (name, ad_type, media_url, link_url, skip_after_seconds, start_date, end_date, enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    )
    .bind(data.name, data.adType, data.mediaUrl, data.linkUrl || null, data.skipAfterSeconds, data.startDate, data.endDate, data.enabled ? 1 : 0)
    .run();
  return result.meta.last_row_id;
}

export async function updatePrerollAd(db, id, data) {
  await db
    .prepare(
      "UPDATE preroll_ads SET name = ?, ad_type = ?, media_url = ?, link_url = ?, skip_after_seconds = ?, start_date = ?, end_date = ?, enabled = ? WHERE id = ?"
    )
    .bind(data.name, data.adType, data.mediaUrl, data.linkUrl || null, data.skipAfterSeconds, data.startDate, data.endDate, data.enabled ? 1 : 0, id)
    .run();
}

export async function deletePrerollAd(db, id) {
  await db.prepare("DELETE FROM preroll_ads WHERE id = ?").bind(id).run();
}
