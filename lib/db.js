...
export async function getActiveSponsorAd(db) {
  const rows = await db
    .prepare("SELECT id, title, image_url, link_url FROM sponsor_ads WHERE date(start_date) <= date('now') AND date(end_date) >= date('now') ORDER BY RANDOM() LIMIT 1")
    .all();
...
export async function createSponsorAd(db, data) {
  const result = await db
    .prepare("INSERT INTO sponsor_ads (name, title, image_url, link_url, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?)")
    .bind(data.name, data.title, data.imageUrl, data.linkUrl, data.startDate, data.endDate)
    .run();
...
export async function updateSponsorAd(db, id, data) {
  await db
    .prepare("UPDATE sponsor_ads SET name = ?, title = ?, image_url = ?, link_url = ?, start_date = ?, end_date = ? WHERE id = ?")
    .bind(data.name, data.title, data.imageUrl, data.linkUrl, data.startDate, data.endDate, id)
    .run();
}
